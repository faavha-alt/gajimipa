...
<div class="w-full space-y-6">
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="flex flex-wrap items-end gap-3 border-b border-slate-100 p-4 dark:border-slate-800">
            <div>
                <label for="cari" class="block text-xs font-medium text-slate-500 dark:text-slate-400">Cari deskripsi</label>
                <input
                    id="cari"
                    type="text"
                    wire:model.live.debounce.300ms="search"
                    placeholder="Cari deskripsi..." aria-label="Cari deskripsi..."
            ...
            <div>
                <label for="filter-aktivitas" class="block text-xs font-medium text-slate-500 dark:text-slate-400">Aktivitas</label>
                <select id="filter-aktivitas" wire:model.live="filterAktivitas" aria-label="Filter Aktivitas" class="mt-1 rounded-xl border-slate-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100">
            ...
            @if ($isSuperAdmin)
                <div>
                    <label for="filter-pengguna" class="block text-xs font-medium text-slate-500 dark:text-slate-400">Pengguna</label>
                    <select id="filter-pengguna" wire:model.live="filterUserId" aria-label="Filter Pengguna" class="mt-1 rounded-xl border-slate-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100">
            ...
            <div>
                <label for="dari" class="block text-xs font-medium text-slate-500 dark:text-slate-400">Dari Tanggal</label>
                <input id="dari" type="date" wire:model.live="dari" class="mt-1 rounded-xl border-slate-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100">
            </div>

            <div>
                <label for="sampai" class="block text-xs font-medium text-slate-500 dark:text-slate-400">Sampai Tanggal</label>
                <input id="sampai" type="date" wire:model.live="sampai" class="mt-1 rounded-xl border-slate-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100">
            </div>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse ($logs as $log)
                        <tr class="align-top">
                            ...
                            <td class="whitespace-nowrap px-4 py-3 text-slate-500 dark:text-slate-400">{{ $log->ip_address ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ $isSuperAdmin ? 5 : 4 }}" class="px-4 py-8 text-center text-slate-500 dark:text-slate-400">Belum ada aktivitas yang cocok dengan filter.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
