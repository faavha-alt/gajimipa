...
new #[Layout('layouts.app')] class extends Component
{
    ...
    public function openEdit(int $id): void
    {
        ...
        $this->kode = $bank->kode;
        $this->nama = $bank->nama;
        $this->status_aktif = $bank->status_aktif;
        $this->showModal = true;
    }
    ...
    public function save(): void
    {
        ...
        Bank::updateOrCreate(['id' => $this->editingId], $validated);

        $this->showModal = false;
        session()->flash('status', $this->editingId ? 'Bank berhasil diperbarui.' : 'Bank berhasil ditambahkan.');
        $this->reset(['editingId', 'kode', 'nama']);
    }
    ...
    public function toggleActive(int $id): void
    {
        ...
        $bank = Bank::findOrFail($id);
        $bank->update(['status_aktif' => ! $bank->status_aktif]);
    }
    ...
    public function delete(int $id): void
    {
        ...
        $bank = Bank::withCount('employees')->findOrFail($id);

        if ($bank->employees_count > 0) {
            session()->flash('error', 'Bank "'.$bank->nama.'" tidak bisa dihapus karena masih dipakai '.$bank->employees_count.' pegawai. Nonaktifkan saja jika sudah tidak dipakai.');

            return;
        }

        $bank->delete();
        session()->flash('status', 'Bank berhasil dihapus.');
    }

    public function with(): array
    {
        return [
            'banks' => Bank::withCount('employees')
