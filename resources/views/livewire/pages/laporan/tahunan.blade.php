...
<div class="w-full space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex gap-2">
            <a href="{{ route('laporan.tahunan-pdf', $tahun) }}" target="_blank">
                <x-secondary-button type="button">Download PDF</x-secondary-button>
            </a>
            <a href="{{ route('laporan.tahunan-excel', $tahun) }}">
        ...
        <div class="divide-y divide-slate-100 p-5 dark:divide-slate-800">
            ...
            @forelse ($perBulan as $row)
                <div class="flex items-center gap-4 py-3">
                    ...
                    <span class="w-16 shrink-0 text-right text-xs text-slate-400">{{ $row['jumlah_pegawai'] }} pegawai</span>
                </div>
            @empty
                <p class="py-10 text-center text-sm text-slate-400">Belum ada periode FINAL/ARSIP pada tahun ini.</p>
            @endforelse
        </div>
    </div>
</div>
